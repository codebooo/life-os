...
/**
 * Fires the reminder: a full-screen-intent notification that launches
 * [AlarmActivity] over the lockscreen. The looping alarm sound + vibration
 * live in that activity (on the ALARM stream, so silent mode can't mute it) —
 * no foreground service, which Android 14 can refuse to start from a
 * background alarm broadcast.
 *
 * Channel history: v1 (alpha.1) was created silent and channels are immutable,
 * so v2 is a fresh id with an alarm sound for the heads-up case.
 */
@AndroidEntryPoint
class ReminderAlarmReceiver : BroadcastReceiver() {
    ...
    private fun showNotification(context: Context, reminderId: Long, titleHint: String?) {
        ...
        if (!manager.areNotificationsEnabled()) {
            LifeLogger.e(TAG, "Notifications disabled — reminder $reminderId may be silent")
        }
