...
object NaturalTimeParser {
    ...
    fun parse(text: String, now: Long = System.currentTimeMillis()): Long? {
        ...
        RELATIVE.find(lower)?.let { match ->
            ...
            val unitMillis = when (match.groupValues[2].first()) {
                'm' -> 60_000L
                'h' -> 3_600_000L
                'd' -> 86_400_000L
                else -> return@let
    ...
    // "in 30 minutes" but also compact forms with no "in": "1m", "5min", "2h", "1d".
    // The unit letter must not run into another letter, so "6pm"/"5 mangoes" stay untouched.
